...
export const verifyRequest = asyncWrapper(async (req, res, next) => {
  const keyHeader = req.headers['x-key'] as string
  const keyQuery = req.query['x-key'] as string
...
export const sessionVerification = asyncWrapper(async (req, res, next) => {
  const { verificationStatus, blockerDetails } = req.user?.trainerInfo
  if (verificationStatus !== ACTIVE_STATUS) {
    return returnResponse(res, {
      ok: false,
      status: 401,
      data: {
      ...
      message: `Looks like your status is still ${verificationStatus.toLocaleLowerCase()}, please verify your details or contact admin for further information`
    })
  }
  next()
})
