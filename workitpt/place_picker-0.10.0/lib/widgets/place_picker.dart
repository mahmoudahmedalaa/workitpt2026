...
class PlacePicker extends StatefulWidget {
  ...
  final Future<String?> Function(LatLng latLng)? onLocationWillSelect;
  final void Function(String message)? onSelectionRejected;
...
class PlacePickerState extends State<PlacePicker> {
            ...
            if (!this.hasSearchTerm)
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    SelectPlaceAction(getLocationName(), () async {
                      final status = await widget.onLocationWillSelect
                          ?.call(this.locationResult!.latLng!);
                      if (status != null) {
                        print('locaton error :$status');
                        setState(() {
                          errorMessage = status;
                        });
                        widget.onSelectionRejected?.call(status);
                        return;
                      }
                      setState(() {
                        errorMessage = null;
                        ...
                        children: nearbyPlaces
                            .map((it) => NearbyPlaceItem(it, () {
                                  if (it.latLng != null) {
                                    ...
                                    _validateDeliveryArea(it.latLng!);
                                  }
                                }))
                            .toList(),
                      ),
  ...
  void _validateDeliveryArea(LatLng latLng) async {
    errorMessage = await widget.onLocationWillSelect?.call(latLng);
    if (mounted) {
      setState(() {});
    }
  }

  bool isValidDeliveryArea = true;
